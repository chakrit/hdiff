...
The suite opens `hdiff` in a 120-by-36 tmux session, captures unified, vertical-split, and
stacked-split ANSI frames, then captures each Tab-selected fixture file before sending `q`. The
script owns a uniquely named session and removes it through an exit trap, including after a
