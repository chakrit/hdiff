...
use crate::interaction::{self, Input, Transition, Viewport};
...
pub fn run_interactive(output: &[u8]) -> io::Result<()> {
    let mut session = TerminalSession::start()?;
    let result = run_loop(&mut session, output);
...
fn run_loop(session: &mut TerminalSession, output: &[u8]) -> io::Result<()> {
    let lines = output
    ...
    draw(session.writer(), &lines, &viewport, width)?;
    ...
    loop {
        ...
        match interaction::transition(&viewport, input, lines.len()) {
            Transition::Exit => return Ok(()),
            Transition::Redraw(next_viewport) => {
...
fn draw(
    ...
    let visible = lines.iter().skip(viewport.offset).take(viewport.height);
    for line in visible {
